---
title: Integrare Adobe Journey Optimizer
...
La connessione è denominata **[!UICONTROL Connessione abilitata per AJO (*nome sandbox*)]** e contiene i seguenti valori predefiniti per la configurazione e i set di dati:

| **Impostazioni della connessione** | Valore |
|---|---|
| [!UICONTROL Nome connessione] | `AJO Enabled Connection (`_`sandbox name`_`)` |
| [!UICONTROL Descrizione connessione] | [!UICONTROL *Descrivi qui la tua connessione*] |
| [!UICONTROL Tag] | [!UICONTROL *Seleziona tag*] |
...
- Nella scheda **[!UICONTROL Configura]**, i seguenti valori sono configurati come predefiniti.
  ...
  | Impostazioni | Valore |
  |---|---|
  | [!UICONTROL Connessione] | Connessione abilitata per AJO (*nome sandbox*) |
  | [!UICONTROL Nome] | `AJO Enabled Data View (`_`sandbox name`_`)` |
  | [!UICONTROL ID esterno] | `AJO_Enabled_Data_View__`_`sandbox_name`_`_` (derivato dal nome) |
  | [!UICONTROL Descrizione] | `undefined` |
  ...
  | Contenitori | Valore |
  |---|---|
  | [!UICONTROL Nome contenitore persona] | `Person` |
  | [!UICONTROL Nome contenitore sessione] | `Session` |
  | [!UICONTROL Nome contenitore eventi] | `Event` |
...
| Motivo di esclusione messaggi (AJO) | Motivo di esclusione | Set di dati evento di feedback messaggi AJO | `_experience.customerJourneyManagement.`<br/>`messageDeliveryfeedback.`<br/>`messageExclusion.reason` | Tipo di componente: dimensione |
| Categoria errore messaggio (AJO) | Categoria errore | Set di dati evento di feedback messaggi AJO | ` _experience.customerJourneyManagement.`<br/>`messageDeliveryfeedback.`<br/>`messageFailure.category` | Tipo di componente: dimensione |
| Motivo di errore messaggio (AJO) | Motivo di errore | Set di dati evento di feedback messaggi AJO | `_experience.customerJourneyManagement.`<br/>`messageDeliveryfeedback.`<br/>`messageFailure.reason` | Tipo di componente: dimensione |
| Tipo di errore messaggio (AJO) | Tipo di errore | Set di dati evento di feedback messaggi AJO | `_experience.customerJourneyManagement.`<br/>`messageDeliveryfeedback.`<br/>`messageFailure.type` | Tipo di componente: dimensione |
| Stato di errore messaggio (AJO) | Stato di errore | Set di dati evento di feedback messaggi AJO | `_experience.customerJourneyManagement.`<br/>`messageDeliveryfeedback.`<br/>`messageFailure.status` | Tipo di componente: dimensione |
| ID messaggio (AJO) | ID messaggio a cui devono essere correlati i dati. | Set di dati di entità AJO | `_experience.customerJourneyManagement.`<br/>`entities.channelDetails.messageID` | Tipo di componente: dimensione |
| Nuovo tentativo messaggio (AJO) | Numero di tentativi | Set di dati evento di feedback messaggi AJO | `_experience.customerJourneyManagement.`<br/>`messageDeliveryfeedback.retryCount` | Tipo di componente: dimensione |
| ID nodo (AJO) | ID nodo del nodo del percorso. | Set di dati di entità AJO | `_experience.customerJourneyManagement.`<br/>`entities.journey.journeyNodeID` | Tipo di componente: dimensione |
| Nome nodo (AJO) | Nome nodo del nodo del percorso. | Set di dati di entità AJO | `_experience.customerJourneyManagement.`<br/>`entities.journey.journeyNodeName` | Tipo di componente: dimensione |
| Tip di nodo (AJO) | Tipo di nodo del nodo del percorso. | Set di dati di entità AJO | `_experience.customerJourneyManagement.`<br/>`entities.journey.journeyNodeID` | Tipo di componente: dimensione |
| Sistema operativo (AJO) | Nome del sistema operativo. | Set di dati evento esperienza di tracciamento push AJO | `environment.operatingSystem` | Tipo di componente: dimensione |
| Versione sistema operativo (AJO) | Versione del sistema operativo. | Set di dati evento esperienza di tracciamento push AJO | environment.operatingSystemVersion | Tipo di componente: dimensione |
| Piattaforma push (AJO) | Servizio del provider push, ad esempio APN o FCM. | Set di dati evento esperienza di tracciamento e-mail di AJO, Set di dati evento di feedback messaggi di AJO, Set di dati evento esperienza di tracciamento push di AJO | `_experience.customerJourneyManagement.`<br/>`pushChannelContext.platform` | Tipo di componente: dimensione |
| Titolo push (AJO) | Titolo push, non personalizzato. | Set di dati di AJO, Set di dati evento di feedback messaggi di AJO, Set di dati evento esperienza di tracciamento push di AJO | `_experience.customerJourneyManagement.`<br/>`entities.channelDetails.push.title | Component type: Dimension` |
| Criterio di consenso rifiutato (AJO) | Nome del criterio di consenso rifiutato corrispondente. | Eventi passaggio percorso | `_experience.journeyOrchestration.`<br/>`stepEvents.consent.rejectedPolicies.name` | Tipo di componente: dimensione |
| Messaggi SMS in entrata (AJO) | Risposta SMS in entrata, ad esempio Interrompi, Avvia, Abbonati, ecc. | Set di dati evento esperienza di tracciamento e-mail di AJO, Set di dati evento di feedback messaggi di AJO, Set di dati evento esperienza di tracciamento push di AJO | `_experience.customerJourneyManagement.`<br/>`smsChannelContext.inboundMessage` | Tipo di componente: dimensione |
| Tipo di messaggio SMS (AJO) | Provider SMS, ad esempio in entrata, risposta in entrata o invio. | Set di dati evento esperienza di tracciamento e-mail di AJO, Set di dati evento di feedback messaggi di AJO, Set di dati evento esperienza di tracciamento push di AJO | ` _experience.customerJourneyManagement.`<br/>`smsChannelContext.messageType` | Tipo di componente: dimensione |
| Provider SMS (AJO) | Provider SMS, ad esempio Sinch o Twilio. | Set di dati evento esperienza di tracciamento e-mail di AJO, Set di dati evento di feedback messaggi di AJO, Set di dati evento esperienza di tracciamento push di AJO | `_experience.customerJourneyManagement.`<br/>`smsChannelContext.messageType` | Tipo di componente: dimensione |
| Tipo di selezione (AJO) | Superficie di canale su cui è stato visualizzato il messaggio. | Eventi dei passaggi del percorso, Set di dati evento esperienza di tracciamento e-mail AJO, Set di dati evento di feedback messaggi AJO, Set di dati evento esperienza di tracciamento push AJO | `_experience.decisioning.propositions.`<br/>`items.itemSelection.`<br/>`selectionDetail.selectionType` | Tipo di componente: dimensione |
| ID elenco iscrizioni (AJO) | Identificatore univoco per l’elenco iscrizioni. | Set di dati evento esperienza di tracciamento e-mail AJO | `_experience.customerJourneyManagement.`<br/>`messageInteraction.subscription.`<br/>` subscriptionListID` | Tipo di componente: dimensione |
| Superficie (AJO) |  | Eventi dei passaggi del percorso, Set di dati evento esperienza di tracciamento e-mail AJO, Set di dati evento di feedback messaggi AJO, Set di dati evento esperienza di tracciamento push AJO | `_experience.decisioning.`<br/>`propositions.scope` | Tipo di componente: dimensione |
| ID trattamento (AJO) | ID del trattamento selezionato per l’esperimento. | Set di dati di entità AJO | `_experience.customerJourneyManagement.`<br/>`entities.experiment.treatmentID` | Tipo di componente: dimensione |
| Nome trattamento (AJO) | Nome del trattamento selezionato per l’esperimento. | Set di dati di entità AJO | `_experience.customerJourneyManagement.`<br/>`entities.experiment.treatmentName` | Tipo di componente: dimensione |
| ID URL (AJO) | Identificatore univoco dell’URL su cui l’utente ha fatto clic. | Set di dati evento esperienza di tracciamento e-mail AJO | `_experience.customerJourneyManagement.`<br/>`messageInteraction.urlID` | Tipo di componente: dimensione |
| Etichetta URL (AJO) | Etichetta descrittiva dell’URL. | Set di dati evento esperienza di tracciamento e-mail AJO | `_experience.customerJourneyManagement.`<br/>`messageInteraction.label` | Tipo di componente: dimensione |
