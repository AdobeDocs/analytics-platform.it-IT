...
>title="Pubblico medio per minuto del file multimediale"
>abstract="Mostra le prestazioni di contenuti multimediali specifici o in un periodo di tempo personalizzato. Specifica la dimensione di reporting e, facoltativamente, il contenuto del segmento."
...
>[!NOTE]
>
>Il pannello **[!UICONTROL Pubblico medio per minuto]** è disponibile solo per i clienti che hanno acquistato Streaming Media Collection per Customer Journey Analytics.
>
...
Per utilizzare un pannello di **[!UICONTROL Pubblico medio per minuto]**:

1. Crea un pannello per **[!UICONTROL Pubblico medio per minuto]**. Per informazioni su come creare un pannello, consulta [Creare un pannello](panels.md#create-a-panel).
...
1. Con [!UICONTROL **Contenuto specifico**] selezionato nel menu a discesa [!UICONTROL **Calcola metrica per**], seleziona [!UICONTROL **Mostra impostazioni avanzate**], quindi specifica le opzioni di configurazione seguenti:
   ...
   | Opzioni | Descrizione |
   |---------|------------|
   | **[!UICONTROL Impostazioni tabella]** | L&#39;opzione predefinita **[!UICONTROL Mostra valori di calcolo nella tabella]** mostra il numeratore e il denominatore del pubblico medio per minuto come colonne precedenti nella tabella. Deselezionando questa opzione le due colonne vengono rimosse. La colonna del pubblico medio per minuto rimane nella tabella accanto al nome video o all’ID contenuto. |
   | **[!UICONTROL Metrica tempo trascorso]** | È possibile scegliere l&#39;opzione predefinita **[!UICONTROL Tempo contenuto trascorso]**, che include solo il tempo contenuto. Oppure puoi scegliere di utilizzare **[!UICONTROL Tempo trascorso contenuti multimediali]**, che include il contenuto e il tempo degli annunci insieme come calcolo del numeratore per il pubblico medio per minuto. |
...
1. Con [!UICONTROL **Periodo di tempo personalizzato**] selezionato nel menu a discesa [!UICONTROL **Calcola metrica per**], seleziona [!UICONTROL **Mostra impostazioni avanzate**], quindi specifica la seguente opzione di configurazione:
   ...
   | Opzione | Descrizione |
   |---------|------------|
   | **[!UICONTROL Impostazioni tabella]** | L’impostazione predefinita visualizza i valori di calcolo nella tabella, la quale visualizza il numeratore e il denominatore del pubblico medio per minuto come colonne precedenti nella tabella. Deselezionando questa opzione le due colonne vengono rimosse, lasciando solo il pubblico medio per minuto accanto al periodo di tempo. |
...
| Metrica | Descrizione |
|--------|-------------|
| **[!UICONTROL Pubblico medio per minuto]** | Il tempo impiegato per visualizzare il flusso multimediale diviso per la lunghezza del video (durata) fornita tramite Classificazioni. |
...
| Metrica | Descrizione |
|---|---|
| **[!UICONTROL Pubblico medio per minuto]** | Il tempo impiegato per visualizzare il flusso multimediale diviso per la selezione totale o per la granularità selezionata in minuti. |
