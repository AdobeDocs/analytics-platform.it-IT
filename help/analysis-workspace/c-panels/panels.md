...
* per i filtri a discesa che utilizzano elementi dimensionali, fai clic sull’icona a forma di freccia destra accanto alla dimensione desiderata nella barra a sinistra. Questa azione espone tutti gli elementi dimensionali disponibili. Seleziona più elementi dimensionali da questo elenco utilizzando `[Shift + Click]` o `[Ctrl + Click]`, quindi rilasciali nella zona di rilascio del pannello **mentre tieni premuto`[Shift]`**.
* Per i filtri a discesa che utilizzano altri componenti, come metriche, filtri o intervalli di date, seleziona più componenti utilizzando `[Shift + Click]` o `[Ctrl + Click]`. Rilascia la selezione nella zona di rilascio del pannello **mentre tieni premuto`[Shift]`**. In questo contesto, tutti i tipi di componenti vengono trattati come filtri.
* Un singolo filtro a discesa può contenere solo un singolo tipo di componente. Se nella selezione includi più tipi di componenti, viene creato un filtro a discesa separato per ciascun tipo di componente. Ad esempio, se nella selezione includi sia le metriche che gli elementi dimensionali, vengono creati due filtri a discesa separati. Un filtro a discesa include gli elementi dimensionali, l’altro include le metriche.

Per cambiare i dati nel pannello, seleziona una delle opzioni dall’elenco a discesa. Puoi anche scegliere di non filtrare nessuno dei dati del pannello, selezionando **[!UICONTROL No filter]**.
...
* **[!UICONTROL Delete label]**: rimuove il testo sopra un filtro a discesa.
