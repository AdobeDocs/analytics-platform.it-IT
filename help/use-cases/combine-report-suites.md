...
## Problemi relativi alle differenze di schema tra le suite di rapporti
...
Per semplicità, diremo che queste sono le uniche eVar definite per entrambe le suite di rapporti.
...
Questa situazione si tradurrà in relazioni insignificanti per eVar1 e eVar2:

- I campi di eVar conterranno una miscela di valori con significati semantici diversi.
- I termini di ricerca saranno distribuiti tra eVar1 e eVar2.
- Non sarà possibile utilizzare modelli di attribuzione diversi per ciascun termine di ricerca, business unit e categorie di clienti.

## Utilizzo di AEP Data Prep per risolvere le differenze di schema tra le suite di rapporti da utilizzare in CJA

La funzionalità di preparazione dei dati di AEP è integrata con il connettore origine di Analytics e può essere utilizzata per risolvere le differenze di schema descritte nello scenario precedente, dando luogo a eVar con significati coerenti nella visualizzazione dati di CJA. Di seguito è riportata una descrizione di come ciò possa essere realizzato. Le convenzioni di denominazione utilizzate di seguito possono essere personalizzate in base alle tue esigenze.

Prima di creare i flussi di dati della connessione sorgente per la suite di rapporti A e la suite di rapporti B, crea un gruppo di campi personalizzato in AEP (lo chiameremo **Campi unificati** nel nostro esempio) che contiene i campi seguenti:
...
Crea un nuovo schema in AEP (lo chiameremo **Schema unificato** nel nostro esempio). Aggiungi i seguenti gruppi di campi allo schema:
...
Durante la creazione del flusso di dati della connessione di origine per **Suite di rapporti A**, seleziona **Schema unificato** da utilizzare nel flusso di dati. Aggiungi le mappature personalizzate come segue:
...
Nota: Il percorso XDM per i campi di destinazione dipende dalla modalità di configurazione del gruppo di campi personalizzato.

Durante la creazione del flusso di dati della connessione di origine per **Suite di rapporti B**, seleziona di nuovo **Schema unificato** da utilizzare nel flusso di dati. Il flusso di lavoro mostrerà che due campi hanno un conflitto di nome del descrittore. Questo perché i descrittori per eVar1 e eVar2 sono diversi nella suite di rapporti B rispetto alla suite di rapporti A. Ma lo sappiamo già in modo da poter ignorare in modo sicuro il conflitto e utilizzare mappature personalizzate come segue:
...
Crea ora un **Tutte le suite di rapporti** connessione per CJA, che combina il set di dati A e il set di dati B.

Crea un **Vista globale** visualizzazione dati in CJA. Ignorare i campi di eVar originali e includere solo i campi del gruppo di campi Campi unificati.
...
In sostanza, ora hai mappato eVar1 e eVar2 dalle suite di rapporti di origine a tre nuovi campi. Un altro vantaggio dell’utilizzo delle mappature Preparazione dati è che i campi di destinazione ora si basano su nomi significativi dal punto di vista semantico (termine di ricerca, Business Unit, categoria Cliente) invece dei nomi eVar meno significativi (eVar1, eVar2).

Nota: Il gruppo di campi personalizzati Campi unificati e le mappature dei campi associati possono essere aggiunte ai flussi di dati e ai set di dati esistenti del connettore di origine di Analytics in qualsiasi momento, ma questo avrà un impatto solo sui dati successivi.
...
Utilizzando Preparazione dati puoi combinare la categoria del cliente in eVar 1 nei dati di Analytics con la categoria del cliente in Some_field nei dati del call center. Ecco un modo in cui potresti farlo. Anche in questo caso, la convenzione di denominazione può essere modificata in base alle tue esigenze.

Crea un gruppo di campi personalizzato:

| Gruppo di campi personalizzati &quot;Informazioni cliente&quot;  |
| --- |
| Categoria_cliente |

Crea uno schema in AEP. Aggiungi i seguenti gruppi di campi allo schema:
...
Durante la creazione del flusso di dati per **Set di dati A**, seleziona **Schema esteso** come schema. Aggiungi le mappature personalizzate come segue:

| Set di dati Un campo di origine | Campo di destinazione dal gruppo di campi Informazioni cliente |
| --- | --- |
| \_experience.analytics.customDimensions.eVars.eVar2 | _\&lt;path>_.Customer_category |

Durante la creazione del flusso di dati per **Set di dati B**, seleziona di nuovo **Schema esteso** come schema. Aggiungi le mappature personalizzate come segue:
...
La modifica dell’ID componente è una funzione solo di CJA e non influisce sui dati provenienti dal connettore di origine di Analytics che viene inviato al profilo unificato e all’RTCDP.
