...
>abstract="Una modifica dell’ID persona elimina tutti i dati esistenti nella connessione e riacquisisce tutti i dati dal set di dati in base al nuovo ID persona. Questa azione può avere implicazioni in termini di costi. <br/><br/>Quando si seleziona **[!UICONTROL Continue]**, è possibile che il reporting venga posticipato fino al completamento del processo."
...
>title="Cambia ID persistente"
>abstract="Una modifica dell’ID persistente elimina tutti i dati esistenti nella connessione e riacquisisce tutti i dati dal set di dati in base al nuovo ID persistente. Questa azione può avere implicazioni in termini di costi. <br/><br/>Quando si seleziona **[!UICONTROL Continue]**, è possibile che il reporting venga posticipato fino al completamento del processo."
...
>abstract="Seleziona un campo a cui unire uno dei set di dati dell’evento. Se questo elenco è vuoto, probabilmente non hai aggiunto o configurato un set di dati evento."
...
>abstract="Assicurati di aver completato la configurazione del grafo identità prima di utilizzarlo per l&#39;unione delle identità."
...
1. Seleziona uno o più set di dati, quindi seleziona **[!UICONTROL Next]**. Almeno un evento o un set di dati di riepilogo deve far parte della connessione.

1. Configura le impostazioni [ per ciascuno dei set di dati selezionati](#dataset-settings), uno alla volta, nel passaggio ➋ **[!UICONTROL Datasets settings]** della finestra di dialogo **[!UICONTROL Add datasets]**.
...
1. Selezionare **[!UICONTROL Add datasets]** per aggiungere i set di dati configurati alla connessione. Viene inviata una notifica quando non sono state fornite tutte le impostazioni richieste per ciascuno dei set di dati che si desidera aggiungere.
   ...
   In alternativa, è possibile selezionare **[!UICONTROL Cancel]** per annullare l&#39;aggiunta di set di dati alla connessione. In alternativa, selezionare **[!UICONTROL Back]** per tornare al passaggio ➊ **[!UICONTROL Select datasets]**.
...
| Impostazione | Descrizione |
| --- | --- |
| **[!UICONTROL Global Account ID]** | Seleziona un ID account globale (l’identificatore univoco di un account) dalle identità disponibili definite nello schema del set di dati in Experience Platform. Applicabile quando hai aggiunto l’account globale come contenitore alla connessione. <p>Se un record non contiene un valore per l’identità selezionata come ID account per il set di dati dell’evento, il record viene ignorato. |
| **[!UICONTROL Account ID]** | Seleziona un ID account (l’identificatore univoco di un account) dalle identità disponibili definite nello schema del set di dati in Experience Platform. Applicabile quando non hai aggiunto l’account globale come contenitore alla connessione. |
...
|---|---|
| **[!UICONTROL Key]** | Chiave da utilizzare per un set di dati di ricerca. <p>Se un record non contiene un valore per la chiave selezionata per il set di dati di ricerca, il record viene ignorato. |
...
Per un’anteprima della connessione creata, seleziona ![PageSearch](/help/assets/icons/PageSearch.svg) **[!UICONTROL Connection preview]** nella finestra di dialogo delle impostazioni della connessione.
...
Per visualizzare una mappa delle relazioni tra i set di dati inclusi nella connessione, seleziona ![GraphPathing](/help/assets/icons/GraphPathing.svg) **[!UICONTROL Connection map]** nella finestra di dialogo delle impostazioni della connessione.
