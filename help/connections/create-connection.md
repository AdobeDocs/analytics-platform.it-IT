...
1. In CJA, fai clic sulla scheda Connessioni .
...
1. Configura le impostazioni di connessione.
   ...
   | Aggiungi set di dati (vedi di seguito) | Aggiungi i set di dati se nell’elenco dei set di dati non sono presenti set di dati. |
   | Nome set di dati | Seleziona uno o più set di dati da richiamare nel Customer Journey Analytics e fai clic su **[!UICONTROL Add]**.<p>Se hai molti set di dati tra cui scegliere, puoi cercare quelli giusti utilizzando la barra di ricerca per i set di dati di Ricerca sopra l’elenco dei set di dati. |
   | Ultimo aggiornamento | Solo per i set di dati evento, questa impostazione viene impostata automaticamente sul campo timestamp predefinito dagli schemi basati su eventi in Experience Platform. &quot;N/D&quot; significa che questo set di dati non contiene dati. |
   | Schema | questo è lo [schema](https://experienceleague.adobe.com/docs/experience-platform/xdm/schema/composition.html?lang=en) in base a cui il set di dati è stato creato in Adobe Experience Platform. |
   | Tipo di set di dati | Per ogni set di dati aggiunto alla connessione, Customer Journey Analytics imposta automaticamente il tipo di set di dati in base ai dati in arrivo. Esistono 3 tipi di set di dati diversi: Dati evento, dati profilo e dati di ricerca. Vedi la tabella seguente per una spiegazione dei tipi di set di dati. |
   | ID persona | seleziona un ID persona dall’elenco a discesa delle identità disponibili. Queste identità sono state definite nello schema del set di dati in Experience Platform. Vedi di seguito per informazioni su come utilizzare Identity Map come ID persona.<p>IMPORTANTE: Se non è presente alcun ID persona tra cui scegliere, significa che uno o più ID persona non sono stati definiti nello schema. Visualizza [questo video](https://www.youtube.com/watch?v=G_ttmGl_LRU) su come definire un’identità in Experience Platform. |
   | Chiave | Solo per i set di dati di ricerca (ad esempio _id). |
   | Chiave di corrispondenza | Solo per i set di dati di ricerca (ad esempio _id). |
   | Importare nuovi dati | Impostare su On o Off. |
   | Dati di backfill |  |
   | Stato del backfill | Indica se vengono elaborati dati di backfill. |
...
1. Ora configura i set di dati uno per uno.
   ...
   | Marca temporale | Solo per i set di dati evento, questa impostazione viene impostata automaticamente sul campo timestamp predefinito dagli schemi basati su eventi in Experience Platform. |
   | Importare nuovi dati | Seleziona questa opzione se desideri stabilire una connessione continua in modo che tutti i nuovi batch di dati aggiunti ai set di dati in questa connessione passino automaticamente in Workspace. Può essere impostato su On o Off. |
   | Recupero del set di dati | Fai clic su **[!UICONTROL Request backfill]** per eseguire il backfill dei dati storici.<ul><li>È possibile retrocompilare ogni set di dati singolarmente.</li><li>Diamo priorità ai nuovi dati aggiunti a un set di dati nella connessione in modo che questi dati abbiano la latenza più bassa.</li><li>Eventuali dati di retrocompilazione (storici) vengono importati a una velocità più bassa. La latenza è influenzata da quanti dati storici si hanno.</li><li>Il Connettore sorgente Adobe Analytics importa fino a 13 mesi di dati, indipendentemente dalle dimensioni.</li></ul> |
   | Stato del backfill | Gli indicatori di stato possibili sono:<ul><li>Operazione riuscita</li><li>Elaborazione di X backfill</li><li>Disattivato</li></ul> |
   | ID set di dati | questo ID viene generato automaticamente. |
   | Descrizione | Descrizione data alla creazione del set di dati. |
   | Dimensione del set di dati | Dimensione del set di dati. |
   | Schema | questo è lo schema in base a cui il set di dati è stato creato in Adobe Experience Platform. |
   | Set di dati | Nome del set di dati. |
   | Anteprima: `<dataset name>` | Visualizza in anteprima il set di dati con le colonne data, ID e ID . |
   | Rimuovi | Rimuovi questo set di dati dalla connessione. |
