...
- **Usa connettore sorgente API HTTP** per inviare facilmente i dati nel set di dati configurato in Adobe Experience Platform.
...
>[!NOTE]
>
>Questa è una guida semplificata su come inserire dati in streaming in Adobe Experience Platform e utilizzarli nel Customer Journey Analytics.  Si raccomanda vivamente di studiare le informazioni aggiuntive quando si fa riferimento a tali informazioni.
...
5. In [!UICONTROL Field Properties] pannello, immetti `Identification` come nome, **[!UICONTROL Identification]** come [!UICONTROL Display name], seleziona **[!UICONTROL Object]** come [!UICONTROL Type] e seleziona **[!UICONTROL Profile Core v2]** come [!UICONTROL Field Group].
   ...
   Questo aggiungerà funzionalità di identificazione allo schema. Nel tuo caso, vuoi identificare le informazioni sulla fidelizzazione utilizzando l’indirizzo e-mail nei tuoi dati batch.
...
## Configurazione di una connessione in streaming API HTTP

L&#39;applicazione di origine esegue lo streaming dei dati conformi allo schema creato e al relativo aspetto.
...
Per inviare questi dati al set di dati creato, devi definire un endpoint di streaming a cui inviare i dati. A tale scopo, definisci un connettore di origine API HTTP.
...
3. Seleziona **Configurazione** in [!UICONTROL HTTP API] piastrelle.
   ...
   ![Configurazione del riquadro API HTTP](./assets/httpapi-tile.png)
...
4. In [!UICONTROL Authentication] fase [!UICONTROL Add data] schermo:
   ...
   Seleziona **[!UICONTROL XDM compatible]** per indicare che i dati che eseguirai lo streaming sono compatibili con uno schema XDM esistente.
...
5. In [!UICONTROL Dataflow detail] fase [!UICONTROL Add data] schermata:

   Seleziona **[!UICONTROL Existing dataset]**, seleziona il set di dati dall’elenco e dai un nome al [!UICONTROL Dataflow name].
...
7. Vedrai la definizione finale dell’endpoint di streaming API HTTP.
