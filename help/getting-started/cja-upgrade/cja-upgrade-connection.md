...
1. Configura le impostazioni della connessione.
   ...
   | **[!UICONTROL Aggiungi set di dati]** (vedi sotto) | Aggiungi i set di dati se nell’elenco dei set di dati non sono presenti set di dati. |
   ...
   | **[!UICONTROL Tipo set di dati]** | Per ogni set di dati aggiunto alla connessione, Customer Journey Analytics imposta automaticamente il tipo di set di dati in base ai dati in arrivo. Esistono 3 tipi diversi di set di dati: dati evento, dati profilo e dati di ricerca. Nella tabella seguente puoi trovare una spiegazione dei tipi di set di dati. |
   ...
   | **[!UICONTROL ID persona]** | Seleziona un ID persona dal menu a discesa delle identità disponibili. Queste identità sono state definite nello schema del set di dati in Experience Platform. Vedi di seguito per informazioni su come utilizzare Identity Map come ID persona.<p>IMPORTANTE: se non è presente alcun ID persona tra cui scegliere, significa che uno o più ID persona non sono stati definiti nello schema. Guarda [questo video](https://www.youtube.com/watch?v=G_ttmGl_LRU) su come definire un’identità in Experience Platform. |
   | **[!UICONTROL Chiave]** | Solo per i set di dati di ricerca (ad esempio _id). |
   | **[!UICONTROL Chiave corrispondente]** | Solo per i set di dati di ricerca (ad esempio _id). |
   | **[!UICONTROL Importa nuovi dati]** | Imposta su On o Off. |
   | **[!UICONTROL Dati backfill]** | Puoi richiedere di eseguire la retrocompilazione dei dati in un set di dati. Ad esempio, puoi richiedere di eseguire la retrocompilazione degli ultimi 7 giorni di dati. Configura il set di dati correttamente e verifica la connessione. Se tutto sembra a posto, puoi eseguire facilmente il backfill di tutti i dati rimanenti.<p>Inoltre, puoi abilitare l’importazione di nuovi dati per set di dati. |
   | **[!UICONTROL Stato backfill]** | Questo stato indica se vengono elaborati dati di retrocompilazione. |
...
1. Nella finestra di dialogo delle impostazioni della connessione, seleziona **[!UICONTROL Aggiungi set di dati]**.
...
1. Ora configura singolarmente ciascun set di dati.
   ...
   | **[!UICONTROL Chiave]** | Disponibile solo per i set di dati di ricerca. Chiave da utilizzare per un set di dati di ricerca. |
   | **[!UICONTROL Chiave corrispondente]** | Disponibile solo per i set di dati di ricerca. Chiave corrispondente per partecipare a uno dei set di dati dell’evento. Se questo elenco è vuoto, probabilmente non hai aggiunto o configurato un set di dati evento. |
   ...
   | **[!UICONTROL Descrizione]** | Descrizione specificata quando è stato creato il set di dati. |
   | **[!UICONTROL Dimensioni set di dati]** | Dimensione del set di dati. |
